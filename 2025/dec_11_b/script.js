...
const getConnectionCount = (indata, startpoint, endpoint, throughpoints) => {
    const paths = [];
    ...
    for (let output of startingPoint.outputs) {
        paths.push([startingPoint.device, output]);
    }
    const pathways = getpathways(paths, connections, endpoint);
    const correctPathCount = getCorrectPathCount(pathways, throughpoints);
    return correctPathCount;
}

const getCorrectPathCount = (pathways, throughpoints) => {
    let correctPathCount = 0;
    for (let pathway of pathways) {
        let correct = 0;
        for (let point of throughpoints) {
            if (pathway.includes(point)) {
                ++correct;
            }
        }
        if (correct === throughpoints.length) {
            ++correctPathCount;
        }
    }
    return correctPathCount;
...
const getpathways = (pathways, connections, endpoint) => {
    let running = true;
    do {
...
const main = () => {
    app.testAnswer = getConnectionCount(testData(), "svr", "out", ["dac", "fft"]);
    app.realAnswer = getConnectionCount(realData(), "svr", "out", ["dac", "fft"]);
    updateTemplate();
}
